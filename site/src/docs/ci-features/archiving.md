...
description: Archiving and reusing builds to run on another machine.
---
...
- Other relevant files:
  ...
  - <!-- md:version 0.9.66 --> Build script output directories for workspace packages that have associated test binaries.
...
```toml title="Extra archive files in `.config/nextest.toml`"
[profile.default]
archive.include = [
...
`--binaries-metadata`
: The path to JSON metadata generated by `cargo nextest list --list-type binaries-only --message-format json`.

`--target-dir-remap`
: A possible new location for the target directory. Requires `--binaries-metadata`.

`--cargo-metadata`
: The path to JSON metadata generated by `cargo metadata --format-version 1`.

    <!-- md:version 0.9.138 --> If the Cargo metadata contains a single default member, e.g., if the metadata was generated with `--manifest-path <path/to/package/Cargo.toml>`, and if `--binaries-metadata` isn't provided, then the build will be anchored to that member's `Cargo.toml` rather than the workspace root.
