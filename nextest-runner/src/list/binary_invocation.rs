...
//! set of environment variables. Buck2, for instance, hands its test executor an
//! `ExternalRunnerTestInfo` whose `command` may place a launcher in front of the
//! binary. This type carries that extra material so the libtest arguments can
//! still be appended to the end.
...
impl TestBinaryInvocation {
    ...
    pub fn is_empty(&self) -> bool {
        self.leading_args.is_empty() && self.env.is_empty()
    }
}
