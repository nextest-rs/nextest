    ...
    pub fn build<'a>(
        ...
        TestReporter {
            inner: TestReporterImpl {
                default_filter_config: profile.default_filter().config_name(),
...
impl<'a> TestReporterImpl<'a> {
        ...
        match &event.kind {
            ...
            TestEventKind::TestAttemptFailedWillRetry {
                ...
                if self.status_levels.status_level >= StatusLevel::Retry {
                    ...
                    {
                        self.write_output(test_instance, run_status, true, display_output, writer)?;
                    }
            ...
            TestEventKind::TestFinished {
                ...
                if let Some(display_output) = output_on_test_finished.show_immediate {
                    self.write_output(test_instance, last_status, true, display_output, writer)?;
                }
            ...
            Some(TestExecutionOutput::Output(output)) => {
                ...
                match output {
                    TestOutput::Split { stdout, stderr } => {
                        if !stdout.is_empty() && display_output.stdout {
                            write!(writer, "\n{}", "--- ".style(header_style))?;
                        ...
                        if !stderr.is_empty() && display_output.stderr {
                            write!(writer, "\n{}", "--- ".style(header_style))?;
                            let out_len = self.write_attempt(run_status, header_style, writer)?;
                            // The width is to align test instances.
                            write!(
                    ...
                    TestOutput::Combined { output } => {
                        if !output.is_empty() && display_output.stdout {
                            write!(writer, "\n{}", "--- ".style(header_style))?;
                            let out_len = self.write_attempt(run_status, header_style, writer)?;
                            // The width is to align test instances.
                            write!(
    ...
    fn success_output(&self, test_setting: TestOutputDisplayStreams) -> TestOutputDisplayStreams {
    ...
    if skip_counts.skipped_tests > 0 || skip_counts.skipped_binaries > 0 {
            ...
            && skip_counts.skipped_binaries == skip_counts.skipped_binaries_default_filter
        {
            write!(
                ...
                " {} via {}",
                "skipped".style(styles.skip),
                default_filter_config.style(styles.count)
            )?;
        } else {
                ...
                || skip_counts.skipped_tests_default_filter > 0
            {
                ...
                write!(writer, " via {}", default_filter_config.style(styles.count))?;
            }
        }
        write!(writer, ")")?;
    }
...
struct OutputOnTestFinished {
    ...
    show_immediate: Option<DisplayOutput>,
...
mod tests {
    ...
    fn on_test_finished_show_immediate(
        #[filter(#display.display_output_immediate().is_some())] display: TestOutputDisplayStreams,
    ...
    fn test_write_skip_counts() {
        ...
        }), @" (1 test skipped via profile.my-profile.default-filter)");
        ...
        insta::assert_snapshot!(skip_counts_str(&SkipCounts {
            ...
            skipped_binaries: 0,
            skipped_binaries_default_filter: 0,
        }), @" (2 tests skipped via profile.my-profile.default-filter)");
        ...
        }), @" (1 binary skipped via profile.my-profile.default-filter)");
        ...
        }), @" (2 binaries skipped via profile.my-profile.default-filter)");
        ...
        insta::assert_snapshot!(skip_counts_str(&SkipCounts {
            ...
            skipped_binaries: 1,
            skipped_binaries_default_filter: 1,
        }), @" (1 test and 1 binary skipped via profile.my-profile.default-filter)");
        ...
        }), @" (2 tests and 3 binaries skipped via profile.my-profile.default-filter)");
        ...
        insta::assert_snapshot!(skip_counts_str(&SkipCounts {
            ...
            skipped_binaries: 3,
            skipped_binaries_default_filter: 0,
        }), @" (2 tests and 3 binaries skipped)");
        ...
        insta::assert_snapshot!(skip_counts_str(&SkipCounts {
            ...
            skipped_binaries: 1,
            skipped_binaries_default_filter: 1,
        }), @" (1 test and 1 binary skipped, including 1 binary via profile.my-profile.default-filter)");
        ...
        insta::assert_snapshot!(skip_counts_str(&SkipCounts {
            ...
            skipped_binaries: 1,
            skipped_binaries_default_filter: 0,
        }), @" (3 tests and 1 binary skipped, including 2 tests via profile.my-profile.default-filter)");
    ...
    fn skip_counts_str(skip_counts: &SkipCounts) -> String {
        let mut buf = Vec::new();
        write_skip_counts(
            skip_counts,
    ...
    fn no_capture_settings() {
        ...
        builder
            .set_no_capture(true)
            .set_failure_output(TestOutputDisplayStreams::create_immediate())
            .set_success_output(TestOutputDisplayStreams::create_immediate())
