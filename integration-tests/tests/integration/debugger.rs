...
//! Integration tests for debugger mode.
...
fn test_debugger_integration() {
    ...
    let fake_debugger = fake_debugger_path();
    let fake_debugger_with_args = shell_words::join(
        std::iter::once(fake_debugger.as_str()).chain(["--debugger-arg", "test-value"]),
    );
    ...
    assert!(
        ...
        "should succeed with debugger args: {output}"
    );

    let stderr = output.stderr_as_str();

    // Verify debugger args were passed in.
    assert!(
        stderr.contains("[fake-debugger] found debugger arg: test-value"),
        "stderr should show the debugger arg value was received: {stderr}"
    );
